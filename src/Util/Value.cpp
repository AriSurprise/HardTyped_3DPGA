...
#include "../Type/ang.h"    // unit wrapper of 4-byte angle datum for trig
#include "../File/Log.h"    // System errors, warnings, etc message management
#include "../Math/v2f.h"    // paired float point data (pass-in values, etc)
#include "../Util/Value.h"  // function declaration header file
...
bool QuadraticF(f4 a, f4 b, f4 c, dp::v2f& real, dp::v2f* imag)
{
  ...
  if (NearF(a, 0.0f)) // ~0*x^{2} => not quadratic; can't eventually do '/2a'
  {
    ...
    real.x = real.y = (IsNegF(a) ^ IsNegF(b) ? -INf : INf); 
...
{
  std::string result = IsIntD(value)?
    std::to_string(FloorD(value)) : std::to_string(value);
  return result;
...
0000000000000000000000000";
  std::string result = IsIntF(value)?
    std::to_string(FloorF(value)) : std::to_string(value);
  um i = result.find('.'), len = result.length(), dif = (len-i) % len;
  result = (fracDigits != u1_INVALID)? (i < len)?
    (dif > fracDigits)? result.substr(0, i + fracDigits)
    : result + pad.substr(0, fracDigits - dif)
    : (0 < fracDigits)? result + "." + pad.substr(0, fracDigits)
    : result : result;
  return result;
}// end std::string StrF(f4, u1)                                             */

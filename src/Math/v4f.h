...
#include "../Math/v2f.h"    // Read as plane of <v2f, v2f>; cyclic includes
#include "../Math/v3f.h"    // Read as plane of <v3f, f4>; cyclic includes
...
namespace dp //! Contents for use within DigiPen MAT (& CS) class assignments
{
  ...
  public:
    ...
    struct {
      v2f min; //! 2D range labels first value pair       (also x|r|s|x1 field)
      v2f max; //! 2D range labels second value pair      (also y|g|t|y1 field)
    };
    struct {
    ...
    */
    ...
    /**
    @brief
        Creates a copy of a v4f from source v3f and added dimensional value
    @param source
      - the v3f to used for memberwise <x|r|s, y|g|f4, z|b|p> copying
    @param promotion
      - w|a|q coefficient to append into the field for the next dimension up
    */
    v4f(const v3f& source, f4 promotion = Of);
    ...
    */
    const f4& operator[](s2 channel) const;

...
#^^^^^^^^#^^^^^^^^^#^^^^^^^^^#^^^^^^^^^#^^^^^^^^^#^^^^^^^^^#^^^^^^^^^#^^^^^^^^*/
#pragma once
#include "../Math/v2f.h"        // Class declaration header
...
#include "../Math/m2f.h"        // Generating matrices from vector data
#include "../Math/m3f.h"        // Generating matrices from vector data
#include "../Math/v3f.h"        // Swizzling / promotion operations to larger vectors
#include "../Math/v4f.h"        // Swizzling / promotion operations to larger vectors
...
dp::v4f dp::v2f::AsPoint(f4 z_index) const { return v4f(x, y, z_index, 1.0f); }
// end v4f v2f::AsPoint(f4) const
...
f4 dp::v2f::operator[](s2 channel) const
{
...
} // end f4 v2f::operator[](s2) const


// Get a value from ['x'] or ['y'] channels (or equivalent)                   */
f4& dp::v2f::operator[](s2 channel)
{
  // clamp wildly out of range values into lowest 2 unsigned values
  if (!isascii((s1)channel) /* 'not' 'msb not set' */
    || (channel < 0x20  /* control seq */ && channel > 0xF /* hex range */))

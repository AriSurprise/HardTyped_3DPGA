...
#include "../Util/ASCIIValue.h" // ASCII character value interpretation for operator[]
#include "../Util/Value.h"      // Value manipulation; float point approximation
#include "../Math/v2f.h"        // Union interpretation as set of 3 2D vectors
#include "../Math/v3f.h"        // Union interpretation as set of 2 3D vectors
#include "../Math/v4f.h"        // Extraction / Swizzle of 4D vector subsets
...
namespace dp //! Contents for use within DigiPen MAT (& CS) class assignments
{
  ...
  public:
    ...
    */
    ...
    /** Set the v6f values to <p1.x, p1.y, p1.z, p2.x, p2.y, p2.z> values
    \brief
      - set the v6f values to <p1.x, p1.y, p1.z, p2.x, p2.y, p2.z> values
    \param 1
    ...
    \return
      - reference to the modified v6f for easier subsequent inline action
    */
    inline v6f& Set(const v3f& p1, const v3f& p2)
    {
      st.Set(p1); ed.Set(p2); return *this;
    }
    ...
    */
    inline const f4& operator[](s2 channel) const
    {
      // clamp wildly out of range values into lowest 2 unsigned values
      if (!isascii((s1)channel) /* msb on */
        || (channel < 0x20 /* control seq */
      ...
      { channel = ToNumeral((s1)channel) & SUB_MASK; }
      switch (channel)
      {

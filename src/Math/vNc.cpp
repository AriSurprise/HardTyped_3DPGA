...
// "./src/..."
#include "../File/Log.h"    // System message management for errors, warnings, etc
#include "../Util/Value.h"  // Basic value comparisons: min, max, clamp, near, etc
#include "../Math/vNf.h"    // Inner product results in list of terms over the real
  ...
  : dimens(dimensions)
{
  if (dimens != 0)
  {
    elem = new C010[dimens];
    if (elem)
    {
      for (size_t i = 0; i < dimens; ++i) { elem[i] = nullC010; }
    }
    else
    {
  ...
  : dimens(dimensions)
{
  if (dimens != 0)
  {
    ...
    if (elem)
    {
      ...
      {
        elem[k] = C010((w * static_cast<f4>(k)));
      }
    }
    else
    {
...
dp::vNc::vNc(std::initializer_list<C010> init)
  : dimens(init.size())
{
  elem = new C010[dimens];
  if (elem)
  ...
  {
    Log::Error("vNc(initializer_list<C010>) - allocation failed for "
      + std::to_string(dimens) + "element list");
    dimens = 0;
    elem = nullptr;
  }
...
{
  um longer = MaxUM(dimens, other.dimens);
  um shorter = MinUM(dimens, other.dimens);
  vNc result(static_cast<u4>(longer));
  for (um i = 0; i < shorter; ++i)
  {
    result.elem[i] = elem[i] * other.elem[i].Conjugate(); // set products
  }
  for (um i = shorter; i < longer; ++i)
  {
    result.elem[i] = nullC010; // 0*x = 0 = x*0 for any length discrepancies
  }
  return result;
...
dp::C010 dp::vNc::InnerProduct(const vNc& other) const
{
  C010 productSum = 0.0f;
...
dp::vNc& dp::vNc::SetProduct(const vNc& other)
{
  ...
  for (um i = 0; i < shorter; ++i) { elem[i] *= (other.elem[i]); }
  return *this;
...
const dp::C010& dp::vNc::operator[](size_t index) const
{
...
dp::C010& dp::vNc::operator[](size_t index)
{
  if (index >= dimens) { index %= dimens; }
  return elem[index];
...
dp::C010 dp::vNc::operator*(const vNc& other) const
{
  C010 productSum = 0.0f;
...
} // end C010 operator*(const vNc&) const
...
bool dp::operator==(const vNc& lhs, const vNc& rhs)
{
  if (lhs.Dimens() == rhs.Dimens())
  {
    ...
    {
      if (!NearF(lhs[i].real, rhs[i].real)
       || !NearF(lhs[i].imag, rhs[i].imag)) { return false; }
    }
    return true;
  }
  return false;

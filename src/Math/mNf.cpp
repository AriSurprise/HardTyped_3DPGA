...
#include "../Math/mNf.h"  // Associated class declaration header
...
dp::mNf::mNf(u2 dimens, f4 scalar) : cell(dimens)
{
  // Initialize to identity / standard basis for given dimensions
  for (um c = 0; c < Dimens(); ++c) // Per column...
  {
...
dp::mNf dp::mNf::Minor(u2 row, u2 col) const
{
  mNf result((u2)(Dimens()-1));
...
f4 dp::mNf::MinorDet(u2 row, u2 col) const
{
...
dp::mNf& dp::mNf::Resize(u2 squareDimensions, f4 traceVal, f4 elseVal)
{
  if ((um)squareDimensions != Dimens())
  {
    cell.resize((um)squareDimensions);
    for (um c = 0; c < Dimens(); ++c)
    {
...
std::string dp::mNf::ToString(bool lineBreaks) const
{
  ...
  std::string result("[[" + std::to_string(mt[0][0]));
  for (um c = 1; c < Dimens(); ++c)
  {
    result += ", " + std::to_string(mt[0][c]);
  }
  for (um r = 1; r < Dimens(); ++r)
  {
    result += end + std::to_string(mt[r][0]);
    for (um c = 1; c < Dimens(); ++c)
    {
      result += ", " + std::to_string(mt[r][c]);
    }
  }
  result += "]]";
  return result;
...
{
  mNf copy((u2)Dimens());
  for (um c = 0; c < Dimens(); ++c) // In each column vector...
  {
    for (um r = 0; r < Dimens(); ++r) // Copy each row element, negated
    {
...
const dp::vNf& dp::mNf::operator[](um subscript) const
{
  subscript %= Dimens();
...
// Get a column v4f from the m4f's cell[0-3] values: <0:u, 1:v, 2:n, 3:t>     */
dp::vNf& dp::mNf::operator[](um subscript)
{
  subscript %= Dimens();
  return cell[subscript];
...
dp::vNf dp::mNf::operator*(const vNf& vector) const
{
  mNf mt(this->Transposed());
  vNf result(Dimens());
  um min = MinUM(Dimens(), vector.Dimens());
  um max = MaxUM(Dimens(), vector.Dimens());
  for (um r = 0; r < min; ++r) { result[r] = mt[r].Dot(vector); }
...
{
  for (um i = 0; i < Dimens(); ++i) { cell[i] *= scalar; }
  return *this;
  ...
  vectorSet) : basis(basisSet), augment(vectorSet)
{
  ...
  um max = SquareDimens();
  for (um c = 0; c < AugmentDimens(); ++c)
  {
    if (max < augment[c].Dimens()) { max = augment[c].Dimens(); }
  }
  if (max > SquareDimens()) { basis.Resize((u2)max); }
  for (um c = 0; c < AugmentDimens(); ++c)
  {
...
um dp::BasisShiftNf::AugmentDimens(void) const
{
  return static_cast<um>(augment.size());
}

um dp::BasisShiftNf::Columns(void) const
{
  return SquareDimens() + AugmentDimens();
}

um dp::BasisShiftNf::SquareDimens(void) const
{
  return basis.Dimens();
}

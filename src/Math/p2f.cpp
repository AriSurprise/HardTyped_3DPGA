...
#include "../Math/p2f.h"        // Class declaration header
...
bool dp::operator==(const p2f& lhs, const p2f& rhs)
{
  ...
  if (NearF(AbsF(lhs.radius), AbsF(rhs.radius), PROXIMITY))
  {
    f4 lha = ModF(lhs.a.r(), TAU, 0.0f),
      rha = ModF(rhs.a.r(), TAU, 0.0f);
...
bool dp::operator!=(const p2f& lhs, const p2f& rhs)
{
  ...
  if (NearF(AbsF(lhs.radius), AbsF(rhs.radius), PROXIMITY))
  {
    f4 lha = ModF(lhs.a.r(), TAU),
       rha = ModF(rhs.a.r(), TAU);
    if ((lhs.radius > 0.0f && rhs.radius > 0.0f)
     || (lhs.radius < 0.0f && rhs.radius < 0.0f))
    {
      return (lha != rha);

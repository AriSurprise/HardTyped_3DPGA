...
// "./src/..."
#include "../Math/vSpace.h"     // Generic cartesian plane definitions
#include "../Type/ang.h"        // Angle extraction requires arc unit parameter
#include "../Type/Errata.h"     // Byte-sized primitive type aliases for brevity
#include "../Util/ASCIIValue.h" // Reading of char range on operator[] calls
#include "../Util/Value.h"      // Value manipulation; float == approximation
...
namespace dp //! Contents for use within DigiPen MAT (& CS) class assignments
{
  ...
  public:
    ...
    struct {
      f4 k;   //! Primary axis normal (e1's dual), right-hand plane area scale
      f4 j;   //! Secondary axis normal (e2's dual), right-hand plane area scale
      f4 i;   //! Tertiary axis normal (e3's dual), right-hand plane area scale
    };
    struct {
      f4 r;   //! 32-bit | HDR RGB red channel value    (exposure [0,1] ratio)
      f4 g;   //! 32-bit | HDR RGB green channel value  (exposure [0,1] ratio)
      f4 b;   //! 32-bit | HDR RGB blue channel value   (exposure [0,1] ratio)
    };
    struct {
    ...
    @brief
      - create a 3 dimensional vector / point (default <0,0,0>)
    @param xir_value
      - (x | i | r | red field value)   - vector primary axis coordinate
    @param yjg_value
      - (y | j | g | green field value) - vector secondary axis coordinate
    @param zkb_value
      - (z | k | b | blue field value)  - vector tertiary axis coordinate
    */
    v3f(f4 xir_value = 0.0f, f4 yjg_value = 0.0f, f4 zkb_value = 0.0f);
    ...
    */
    inline v3f& B(f4 zkb_value)
    { return Z(zkb_value); } // end v3f& v3f::B(f4)                            */
    ...
    @return
      - reference to the modified v3f for easier subsequent inline action
    */
    inline v3f& B(f4 zkb_value, const v2f& clamp)
    { return Z(zkb_value, clamp); } // end v3f& v3f::B(f4, const v2f&)         */
    ...
    */
    ...
    @return
      - reference to the modified v3f for easier subsequent inline action
    */
    inline v3f& G(f4 zkb_value)
    { return Z(zkb_value); } // end v3f& v3f::G(f4)                            */
    ...
    @return
      - reference to the modified v3f for easier subsequent inline action
    */
    inline v3f& G(f4 zkb_value, const v2f& clamp)
    { return Z(zkb_value, clamp); } // end v3f& v3f::G(f4, const v2f&)         */
    ...
    */
    ...
    @return
      - reference to the modified v3f for easier subsequent inline action
    */
    inline v3f& R(f4 xir_value)
    { return X(xir_value); } // end v3f& v3f::R(f4)                            */
    ...
    @return
      - reference to the modified v3f for easier subsequent inline action
    */
    inline v3f& R(f4 xir_value, v2f& clamp)
    { return X(xir_value, clamp); } // end v3f& v3f::R(f4, const v2f&)         */
    ...
    */
    ...
    @return
      - reference to the modified v3f for easier subsequent inline action
    */
    inline v3f& Set(f4 xir_value, f4 yjg_value, f4 zkb_value)
    {
      X(xir_value).Y(yjg_value).Z(zkb_value);
      return *this;
    ...
    */
    ...
    @return
      - reference to the modified v3f for easier subsequent inline action
    */
    v3f& X(f4 xir_value, const v2f& clamp);
    ...
    */
    ...
    @return
      - reference to the modified v3f for easier subsequent inline action
    */
    v3f& Y(f4 yjg_value, const v2f& clamp);
    ...
    */
    ...
    @return
      - reference to the modified v3f for easier subsequent inline action
    */
    v3f& Z(f4 zkb_value, const v2f& clamp);
    ...
    */
    inline f4 operator[](s2 channel) const
    {
      ...
      else if (IsDigit((s1)channel))
      {
        channel = ToNumeral((s1)channel) & 3; // => %4, ie between [0,111b]

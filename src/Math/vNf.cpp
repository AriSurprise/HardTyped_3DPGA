...
#include "../Math/mNf.h"    // N-D vector dot transpose to get symmetrical matrix
...
dp::vNf::vNf(um dimensions, f4 value)
  : dimens(dimensions)
{
  if (dimens != 0)
  {
    ...
    if (elem)
    {
      for (um i = 0; i < dimens; ++i) { elem[i] = value; }
    }
    else
    {
      Log::Error("vNf(um,f4) - allocation failed for "
...
dp::vNf::vNf(std::initializer_list<f8> init)
  : dimens(static_cast<u4>(init.size()))
{
  elem = new f4[dimens];
  if (elem)
...
dp::vNf& dp::vNf::Resize(um dimensions, f4 value)
{
    ...
    else
    {
      ...
      else
      {
        if (dimens < dimensions) // elongation request
        {
          ...
          for (um i = dimens; i < dimensions; ++i)
          {
            elem[i] = value;
          }
        }
...
const f4& dp::vNf::operator[](um index) const
{
  if (index >= dimens) { index %= dimens; }
  return elem[index];
} // end f4 operator[](um index) const


// Get a reference to the given [0, n-1] index subscript of the vector        */
f4& dp::vNf::operator[](um index)
{
  if (index >= dimens) { index %= dimens; }
  return elem[index];
} // end f4& operator[](um index)
...
{
  for (um i = 0; i < dimens; ++i) { elem[i] *= scalar; }
  return *this;
} // end vNf& operator*=(f4)

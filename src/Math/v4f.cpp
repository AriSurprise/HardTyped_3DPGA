...
#include "../Type/ang.h"        // Unit measures for conversion in and out of arc spans
#include "../Util/ASCIIValue.h" // char range based [] operator manipulation
#include "../Math/m3f.h"        // Generating matrices from vector data
#include "../Math/m4f.h"        // Generating matrices from vector data
#include "../Util/Value.h"      // NearF() float point (proximity as equality) function
#include "../Math/v2f.h"        // Swizzling operations to truncate to smaller vectors
#include "../Math/v3f.h"        // Swizzling operations to truncate to smaller vectors
#include "../Math/v4f.h"        // Class declaration header
...
dp::v4f::v4f(const v3f& source, f4 promotion)
  : v4f(source.x, source.y, source.z, promotion)
{ } // end v4f::v4f(const v3f&,(f4))                                               */
...
const f4& dp::v4f::operator[](s2 channel) const
{
  // bitmask wildly out of range values into lowest 4 unsigned values
  if (!isascii((s1)channel) /* 'not' 'msb not set' */
  ...
  { channel = ToNumeral((s1)channel) & VSUBS_MASK; }
...
} // end const f4& v4f::operator[](s2) const                                  */


f4& dp::v4f::operator[](s2 channel)
{
